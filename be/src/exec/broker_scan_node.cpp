...
std::unique_ptr<ScannerInterface> BrokerScanNode::create_scanner(const TBrokerScanRange& scan_range,
    ...
    std::unique_ptr<ScannerInterface> scanner(scan);
    return scanner;
}
...
Status BrokerScanNode::scanner_scan(
    ...
    std::unique_ptr<ScannerInterface> scanner = create_scanner(scan_range, counter);
